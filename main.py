...
def fetch_from_tavily(query):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            return f"❌ Unable to fetch information from Tavily for '{query}'."
...
model = genai.GenerativeModel(
    ...
    system_instruction=(
        "You are Latesty, a helpful assistant that provides the latest information, trends, and updates on any topic the user requests. "
        "If the user asks about 'latest' or 'trending', search using Tavily and summarize the latest content. "
        "Provide helpful, clear, and simple explanations. Use user memory to tailor responses."
    ),
)
...
def chatbot_response(user_input, user_id="user_1"):
    ...
    """
    try:
        # 🔥 Handle "latest" or "trending" queries using Tavily
        if "latest" in user_input.lower() or "trending" in user_input.lower():
            ...
            if isinstance(tavily_response, list) and len(tavily_response) > 0:
                message_to_gemini = "Summarize the following latest updates:\n"
                ...
                return "❌ No latest updates found from Tavily."
...
# 🔥 **Main Streamlit UI**
st.set_page_config(
    page_title="Latesty Bot",
    page_icon="🤖"
)

st.title("🤖 **Latesty Bot** ")
